...
// Newsletter webhook endpoint for SMS trigger
app.post('/api/newsletter-webhook', async (req, res) => {
  const { firstName, lastName, email, phone, preferredContact } = req.body;
  console.log('📩 Newsletter webhook received:', req.body);

  if (preferredContact === 'phone' && phone) {
    // Simulate sending SMS (replace with real SMS API integration)
    console.log(`📲 Sending SMS to ${phone}: Hello ${firstName}, thanks for subscribing to DevTone Agency!`);
    // Here you would integrate with Twilio, Vonage, etc.
    return res.status(200).json({ success: true, message: 'SMS sent (simulated)' });
  }

  return res.status(200).json({ success: true, message: 'No SMS sent (not phone method)' });
});
